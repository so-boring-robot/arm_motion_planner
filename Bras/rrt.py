...
class RRT:
    ...
    def getPathToTarget(self):
        if self.path[len(self.path)-1].previous==None:
            return
        if self.path[len(self.path)-1].previous.toString() is not None:
            print(self.path[len(self.path)-1].previous.toString())
...
if __name__ == '__main__':
    ...
    start = node.Node(340, 148)
    end = node.Node(518, 99)
    obstacles = [node.Node(0,0)]
