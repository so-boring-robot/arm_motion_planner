...
if __name__ == '__main__':
    ...
    lengths = [95,110,100]
    pygame.init()
    width, height = sum(lengths)*2, sum(lengths)
    ...
    while running:
        ...
        if(target!=None):
            ...
            for i in range(len(robot_arm.joint_positions)-1): 
                data_reversed.append(180-abs(round(calculer_angle_entre_points(robot_arm.joint_positions[i].getCoordinate(), robot_arm.joint_positions[i+1].getCoordinate()))))  
                
            data_reversed_save = data_reversed[1]
            data_reversed[1] -= data_reversed[0]
            
            data_reversed[2] -= data_reversed_save
                    
            dataset += reversed(data_reversed)
            dataset.append(90)
            print(dataset)
            s = 0
            #for pos in dataset:
